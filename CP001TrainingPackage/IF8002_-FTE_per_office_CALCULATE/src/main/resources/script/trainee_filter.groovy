...
def String Filter_trainees(String FTE,String s_customDate10,String s_reference,String s_custTypeDienst){
...
//Calculate b_receiveParticipations and b_receiveTopper based on traineemonths
        ...
        if(s_custTypeDienst.equalsIgnoreCase("BINNEN")){
            //participation
            if(( cal_reference.get(Calendar.MONTH) + (12*cal_reference.get(Calendar.YEAR)) ) - ( cal_customDate10.get(Calendar.MONTH) + (12*cal_customDate10.get(Calendar.YEAR))) + i_additionalmonth < 4){
                FTE = 0;
            }
        }else if(s_custTypeDienst.equalsIgnoreCase("BUITEN")){
            if(( cal_reference.get(Calendar.MONTH) + (12*cal_reference.get(Calendar.YEAR)) ) - ( cal_customDate10.get(Calendar.MONTH) + (12*cal_customDate10.get(Calendar.YEAR))) + i_additionalmonth < 2){
                FTE = 0;
            }
        }else{
            FTE=FTE
